...
// email is requested.
function noticeEmailTemplate({ heading, body, footer, ctaUrl, ctaLabel }) {
  return `
  <div style="background:#F4F1E8; padding:40px 16px; font-family:'Segoe UI',Arial,sans-serif;">
    <div style="max-width:480px; margin:0 auto; background:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 8px 24px rgba(11,20,16,0.12);">
      <div style="background:linear-gradient(120deg,#0F3D2E,#1B8A63); padding:28px 32px; text-align:center;">
      ...
      <div style="padding:32px;">
        ...
        ${ctaUrl ? `<div style="text-align:center; margin:24px 0 8px;"><a href="${ctaUrl}" style="display:inline-block; background:#0F3D2E; color:#ffffff; text-decoration:none; font-size:14px; font-weight:600; padding:12px 28px; border-radius:999px;">${ctaLabel || 'Open CareerZ.pk'}</a></div>` : ''}
